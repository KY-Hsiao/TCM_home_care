...
```
   ...
   - 這個 npm script 會呼叫 `.codex/scripts/update_web.ps1`，只負責 commit/push，並讓 GitHub push workflow 在線上接續觸發 Vercel；不要從本機直接呼叫 Vercel deploy hook。
...
8. 完整發布流程會：
   ...
   - 若加上 `-NoWaitForGitHubActions`，腳本會在 GitHub push 完成後結束；Vercel 由 GitHub push workflow 在線上接續觸發，不會從本機打 Vercel deploy hook
