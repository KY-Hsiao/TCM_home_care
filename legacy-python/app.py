...
def launch_gui() -> None:
    ...
    root = tk.Tk()
    root.title("TCM Home Care Legacy Demo")
    ...
    title_label = ttk.Label(
        frame,
        text="TCM Home Care Legacy Python 執行面板",
    ...
    description_label = ttk.Label(
        frame,
        text="這是舊版 Python / Tkinter 示範入口，Web MVP 請改用 npm run dev 或 launch_app.ps1。",
        wraplength=500,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TCM Home Care legacy Python 示範程式")
    parser.add_argument(
        "--cli",
        action="store_true",
