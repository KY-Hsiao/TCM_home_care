(() => {
  ...
  function renderPanel() {
    ...
    panel.innerHTML = `
      <div class="line-direct-head">
        ...
        <button type="button" class="line-direct-refresh">重新整理名單</button>
      </div>
    ...
    if (!contacts.length) {
      list.innerHTML = '<div class="line-direct-empty">目前尚未有 LINE 好友名單。請先同步 LINE 官方帳號好友。</div>';
      return;
    }
  ...
  window.addEventListener('storage', (event) => {
    if (event.key === CONTACTS_KEY || event.key === DB_KEY) scheduleRender();
  });
