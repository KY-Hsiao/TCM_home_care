...
function resolveMapQuery({ address, locationKeyword, latitude, longitude }: RouteMapLocation): string {
        ...
        if (!response.ok) {
          lastGeocodeError =
            ...
            (payload?.reason || payload?.status
              ? `Google Geocoding API 回傳 ${payload.reason ?? payload.status}${payload?.error_message ? `：${payload.error_message}` : ""}`
