      ...
      switch (type) {
        case "visit_reminder":
          return {
            eventType: type,
            subject: "明日訪視提醒",
            body: `您好，${detail.patient.name} 明日 ${visitTime} 由 ${detail.doctor.name} 醫師到宅訪視。若有需要調整，請直接由行政端協助處理。`,
                ...
                cardsV2: [
                  {
                    ...
                    card: {
                      header: { title: "明日訪視提醒", subtitle: detail.patient.name },
                      sections: [
                        {
                          widgets: [
        ...
        case "visit_today":
          return {
            eventType: type,
            subject: "今日到訪通知",
            body: `${detail.doctor.name} 醫師已抵達 ${detail.patient.name} 住家，系統已自動記錄抵達時間並開始本次訪視。`,
            templateCode: templateCodeByType[type],
            cardDraft: JSON.stringify(
              {
                cardsV2: [
                  {
                    cardId: "visit_today",
                    card: {
                      header: { title: "今日到訪通知", subtitle: detail.patient.name },
        ...
        case "visit_delay":
          return {
            eventType: type,
            subject: "延遲通知",
            body: `${detail.patient.name} 本次訪視預估延遲 ${delayMinutes ?? 0} 分鐘，若需調整請直接聯繫行政。`,
            templateCode: templateCodeByType[type],
            cardDraft: JSON.stringify(
              {
                cardsV2: [
                  {
                    cardId: "visit_delay",
                    card: {
                      header: { title: "延遲通知", subtitle: detail.patient.name },
        ...
        case "visit_reschedule":
          return {
            eventType: type,
            subject: "請假 / 改期通知",
            body: `${detail.patient.name} 原定訪視需調整。${rescheduleNote ?? "請由行政端確認最新安排。"}。`,
            templateCode: templateCodeByType[type],
            cardDraft: JSON.stringify(
              {
                cardsV2: [
                  {
                    ...
                    card: {
                      header: { title: "請假 / 改期通知", subtitle: detail.patient.name },
        ...
        case "visit_coverage":
          return {
            eventType: type,
            subject: "代班醫師通知",
            body: `${detail.patient.name} 本次訪視改由 ${coverageDoctorName ?? detail.doctor.name} 醫師處理，時間維持 ${visitTime}。`,
            templateCode: templateCodeByType[type],
            cardDraft: JSON.stringify(
              {
                cardsV2: [
                  {
                    ...
                    card: {
                      header: { title: "代班醫師通知", subtitle: detail.patient.name },
        ...
        case "family_followup_normal":
          ...
          return {
            eventType: type,
            subject: "訪視已完成",
            body: `${detail.patient.name} 本次訪視已完成，若需補充說明請由行政端協助處理。摘要：${completionSummary}`,
            templateCode: templateCodeByType[type],
            cardDraft: JSON.stringify(
              {
                cardsV2: [
                  {
                    ...
                    card: {
                      header: { title: "訪視已完成", subtitle: detail.patient.name },
        ...
        case "family_followup_absent":
          return {
            eventType: type,
            subject: "今日未完成看診",
            body: `${detail.patient.name} 今日到訪時未能完成看診，請回覆是否需要協助重新安排。`,
            templateCode: templateCodeByType[type],
        ...
        case "family_followup_admin":
          return {
            eventType: type,
            subject: "行政將協助追蹤",
            body: `${detail.patient.name} 本次案件需要行政協助，若需補充資訊請直接回覆。`,
            templateCode: templateCodeByType[type],
        ...
        case "family_followup_urgent":
          return {
            eventType: type,
            subject: "緊急狀況通知",
            body: `${detail.patient.name} 今日有緊急處置需求，行政將儘速與您聯繫。`,
            templateCode: templateCodeByType[type],
        ...
        case "doctor_departure_check":
          return {
            eventType: type,
            subject: "請確認是否出發",
            body: `${detail.doctor.name} 醫師，請按下出發並開啟定位追蹤，系統會在逼近 ${detail.patient.name} 時提醒您回報狀態。`,
            templateCode: templateCodeByType[type],
        ...
        case "doctor_arrival_feedback":
          return {
            eventType: type,
            subject: "請確認現場狀況",
            body: `系統偵測您已逼近 ${detail.patient.name}，請回報：正常看診 / 不在家 / 行政追蹤 / 緊急處置。`,
            templateCode: templateCodeByType[type],
        ...
        case "doctor_emergency_alert":
          return {
            eventType: type,
            subject: "緊急處置已通知行政",
            body: `${detail.patient.name} 已標記為緊急處置，請優先完成現場處置並等待行政支援。`,
            templateCode: templateCodeByType[type],
        ...
        default:
          return {
            ...
            subject: "訪視完成通知",
            body: `${detail.patient.name} 本次訪視已完成。${(summary ?? detail.patient.last_visit_summary) || "若需補充內容，請由行政端協助處理。"}。`,
                ...
                cardsV2: [
                  {
                    ...
                    card: {
                      header: { title: "訪視完成通知", subtitle: detail.patient.name },
